...
public class AutoServiceImpl implements AutoService{
    ...
    @Override
    public List<Auto> getNoleggio(HttpSession session) {
        if(session.getAttribute("noleggio") != null){
            return (List<Auto>) session.getAttribute("noleggio");
        } else {
            return null;
        }
    }
    ...
    /*
		...
		for(Rental r : vehicle.getRentals())
			if(!r.getReturnTiming().isBefore(rental.getPickupTiming()) &&
				...
				return new ResponseManager("400", "Unacceptable Period").getResponse();
				rental.setUser(user);
		rental.setVehicle(vehicle);
		rental.setState("R");
		rentalDao.save(rental);
		return new ResponseManager("201", "Rental Added").getResponse();
     */
